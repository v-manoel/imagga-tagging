...
class Analyser:
    ...
    def __init__(self) -> None:
        ...
        self.extractor = FormExtractor(AnalyserConfigs.FORM_FILE_PATH)
    ...
    def stack_bar_chart(xvalues: list, yvalues: list[dict], filename: str = "compare_chart") -> None:
        ...
        ax.grid()
        ax1 = ax.bar(bar_l, yvalues[0]['value'], width=bar_width,
                     label=yvalues[0]['label'], color="green", zorder=3)
        ax2 = ax.bar(bar_l, yvalues[1]['value'], bottom=yvalues[0]['value'], width=bar_width,
                     label=yvalues[1]['label'], color="blue", zorder=2)
        ax3 = ax.bar(bar_l, yvalues[2]['value'], bottom=yvalues[1]['value'], width=bar_width,
        ...
        for r1, r2, r3 in zip(ax1, ax2, ax3):
            ...
            plt.text(r2.get_x() + r2.get_width() / 2., h1 + h2 / 2., "%d" % h2,
                     ha="center", va="bottom", color="white", fontsize=10, fontweight="bold")
            plt.text(r3.get_x() + r3.get_width() / 2., h2 + h3 / 2., "%d" % h3,
                     ha="center", va="bottom", color="white", fontsize=10, fontweight="bold")
